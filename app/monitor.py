...
class MarathonMonitor:
    ...
    def collect_characters_info(self, driver: WebDriver, username: str) -> None:
        ...
        try:            
            ...
            if username and characters:
                ...
                if len(characters) == 1 and len(characters[list(characters.keys())[0]]) == 1 and characters[list(characters.keys())[0]][0]['name']:
                    self.db.save_account_data(
                        username=username,
                        alias=characters[list(characters.keys())[0]][0]['name'],
                        server=list(characters.keys())[0]
                    )                    
                    logger.info(f"Выбран дефолтный персонаж: Сервер:{list(characters.keys())[0]} Имя:{characters[list(characters.keys())[0]][0]['name']}")
    ...
    def activate_promo_code(self, promo_code: str, accounts: List[str]) -> Dict[str, int]:
        ...
        """
        # Проверяем существование промокода в базе, если нет - добавляем
        if not self.db.get_promo_code_status(promo_code):
            self.db.save_promo_code_status(promo_code, 'active')  # Добавляем как активный
        activated = 0
        errors = 0
        ...
        for username in accounts:
            ...
            try:
                logger.info(f'Вводим промокод на аккаунта: {username}')
                # Проверяем, был ли уже введен этот промокод на аккаунте
                account_promo_status = self.db.get_account_promo_status(username, promo_code)
                if account_promo_status:
                    # Промокод уже вводился на этом аккаунте
                    if account_promo_status in ['success', 'already_activated']:
                        logger.info(f"Промокод {promo_code} уже был успешно активирован на аккаунте {username} - пропускаем")
                        activated += 1
                        continue
                    elif account_promo_status in ['failed', 'invalid', 'expired']:
                        logger.info(f"Промокод {promo_code} уже вводился на аккаунте {username} с ошибкой {account_promo_status} - пропускаем")
                        errors += 1
                        continue
                ...
                try:
                    ...
                    if error_div:
                        ...
                        logger.error(f"Ошибка активации: {error_text} на аккаунте {username}.\n\t\t\t {error_text}")
                        
                        if "Пин-код уже активирован" in error_text or "Вы уже активировали этот пин-код!" in error_text:
                        ...
                        logger.info(f"Промокод успешно активирован")
    ...
    def _send_gifts_to_game(self, driver: WebDriver, username: str, selections: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if success_msg:
                ...
                requirements_msg = driver.find_elements(By.XPATH, "//div[@id='content_body']/table[@class='tborder']/tbody/tr/td[@class='panelsurround']")
                if requirements_msg and requirements_msg.text:
                    return {'status': 'error', 'message': f'Ошибка перевода на аккаунте {username}: {requirements_msg.text}'}

                error_msg = driver.find_elements(By.XPATH, "//div[@class='m_error']")
                return {'status': 'error', 'message': error_msg[0].text if error_msg else f'Неизвестная ошибка перевода на аккаунте {username}'}
    ...
    def start_scheduled_monitoring(self) -> None:
        ...
        schedule.every(1440).minutes.do(self.check_all_accounts)
