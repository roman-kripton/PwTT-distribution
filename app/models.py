...
class Database:
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    defaults = {
                        ...
                        'mdm_coins': mdm_coins if mdm_coins is not None else None
                    }
    ...
    def get_accounts_for_promo_activation(self, promo_code: str) -> List[str]:
        ...
        try:
            ...
            return []
    def get_account_promo_status(self, username: str, promo_code: str) -> Optional[str]:
        """Получает статус активации промокода для конкретного аккаунта.
        ...
        Returns:
            Статус активации или None если промокод не активировался
        """
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cursor.execute("""
                        SELECT status FROM account_promo_codes 
                        WHERE username = %s AND promo_code = %s
                    """, (username, promo_code))
                    result = cursor.fetchone()
                    return result[0] if result else None
        except Exception as e:
            logger.error(f"Ошибка получения статуса промокода {promo_code} для {username}: {e}")
            return None
